...
if node['cloaker']['url']
  ...
  cloaker_fallback_index = "#{node['cloaker']['web_root']}/#{node['cloaker']['fallback_uri']}"
  ...
  # download URL
  execute "wget --timeout=#{node['cloaker']['wget_network_timeout']} -mkEpnp -nH -H -q -P #{node['cloaker']['wgetdir']} -e robots=off #{node['cloaker']['url']} || true" do
  ...
  ruby_block "move cloaker to fallback path" do
    ...
    only_if { (File.exist?("#{node['cloaker']['wgetdir']}/#{cloaker_index_noext}.html") or \
               File.exist?("#{node['cloaker']['wgetdir']}/#{cloaker_index}") \
              ) and File.exist?(cloaker_index) }
  end
